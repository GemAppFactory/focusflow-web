...
import { Send, Sparkles, User, Bot, Loader2, RotateCcw, CheckCircle, AlertCircle } from 'lucide-react';
...
const TaskNotification: React.FC<{ tasks: Array<{name: string, priority: TaskPriority}>, t: Locales }> = ({ tasks, t }) => {
  const getPriorityIcon = (priority: TaskPriority) => {
    switch(priority) {
      case 'high': return <span className="w-2 h-2 rounded-full bg-red-500 inline-block" />;
      case 'medium': return <span className="w-2 h-2 rounded-full bg-amber-500 inline-block" />;
      case 'low': return <span className="w-2 h-2 rounded-full bg-green-500 inline-block" />;
    }
  };

  const getPriorityLabel = (priority: TaskPriority) => {
    return t.priority[priority].toUpperCase();
  };
  ...
  return (
    <motion.div
      ...
      className="max-w-[85%] ml-10"
    >
      <div className="bg-gradient-to-br from-indigo-500/10 via-purple-500/10 to-indigo-500/10 dark:from-indigo-500/20 dark:via-purple-500/20 dark:to-indigo-500/20 border-2 border-indigo-500/30 dark:border-indigo-400/40 rounded-xl p-4 shadow-lg shadow-indigo-500/10 dark:shadow-indigo-500/20">
        {/* Header */}
        <div className="flex items-center gap-2 mb-3 pb-2 border-b border-indigo-500/20 dark:border-indigo-400/30">
          <Sparkles size={14} className="text-indigo-600 dark:text-indigo-400" />
          <span className="text-[10px] font-bold uppercase tracking-widest text-indigo-700 dark:text-indigo-300">
            AI Generated {tasks.length} {tasks.length === 1 ? 'Task' : 'Tasks'}
          </span>
        </div>
        ...
        <div className="space-y-2 mb-3">
          {tasks.map((task, idx) => (
            ...
            >
              <div className="flex items-center gap-1.5 min-w-[70px] mt-0.5">
              ...
              <span className="text-zinc-800 dark:text-zinc-200 font-medium leading-relaxed flex-1">
                {task.name}
              </span>
            </motion.div>
          ))}
        ...
        {/* Footer */}
        <div className="flex items-center gap-1.5 pt-2 border-t border-indigo-500/20 dark:border-indigo-400/30">
          <CheckCircle size={12} className="text-emerald-600 dark:text-terminal-green" />
          <span className="text-[9px] font-bold text-emerald-700 dark:text-terminal-green uppercase tracking-wider">
            Tasks added to your dashboard
          </span>
        </div>
      </div>
    </motion.div>
...
const AssistantView: React.FC<AssistantViewProps> = ({
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      let updatedMessages = [...newMessages, aiMessage];

      // If tasks were detected, add a task notification message
      if (detectedTasks.length > 0) {
        ...
        updatedMessages.push(taskNotification);
      }
      ...
      // Trigger task addition callback
  ...
  return (
    ...
    >
      ...
      >
        <AnimatePresence>
          {messages.map((msg, i) => {
            // Render task notification
            if (msg.role === 'task-notification' && msg.tasks) {
              return <TaskNotification key={i} tasks={msg.tasks} t={t} />;
            }
